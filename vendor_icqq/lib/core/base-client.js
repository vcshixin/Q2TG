...
class BaseClient extends triptrap_1.Trapper {
    ...
    /** 使用接收到的token登录 */
    async tokenLogin(token = constants_1.BUF0, cmd = 15) {
        ...
        if (token != constants_1.BUF0) {
            ...
            try {
                ...
                if ((String(info) || "").includes("icqq")) {
                    ...
                    this.sig.t10c = info.token_ver >= 4 ? stream.read(stream.read(2).readUInt16BE()) : this.sig.t10c;
                    this.sig.tgtgt = this.sig.t10c?.length ? this.sig.t10c : t119key;
        ...
        cmd = (this.sig.srm_token?.length && this.sig.a1?.length && this.sig.t10c?.length) ? cmd : 11;
        this.sig.tgtgt = cmd === 15 ? this.sig.t10c : (0, constants_1.md5)(this.sig.d2key);
        const t = tlv.getPacker(this);
        const tlvs = [
            t(0x8),
            t(0x18),
    ...
    async writeUni(cmd, body, seq = 0) {
        this.statistics.sent_pkt_cnt++;
        const pkt = await buildUniPacket.call(this, cmd, body, 1, seq);
        if (pkt.length > 0)
            this[NET].write(pkt);
    }
    ...
    async sendOidbSvcTrpcTcp(cmd, body) {
        ...
        const payload = await this.sendUni(cmd, _body);
        //log(payload)
    ...
    sendHeartbeat() {
        return new Promise(async (resolve, reject) => {
            ...
            this.syncTimeDiff();
            this[FN_SEND](await buildUniPacket.call(this, "Heartbeat.Alive", constants_1.BUF0, 0), 10).then(resolve).catch((e) => {
...
async function _register(logout = false, reflush = false) {
    ...
    const pkt = await buildLoginPacket.call(this, "StatSvc.register", body, 1, seq);
    try {
        const payload = await this[FN_SEND](pkt, 6, seq);
        if (logout)
            return 0;
        ...
        if (!result && !reflush) {
            ...
            const heartbeatSuccess = async () => {
                const time = (0, constants_1.timestamp)();
                if ((time - this.sig.hb_time) >= 600) {
                    ...
                    this.sendUni(hb480_cmd, this.sig.hb480(this.uin)).catch(async () => {
...
async function refreshToken(force = false) {
    if ((!this.isOnline() || this.emp_interval === 0 || (0, constants_1.timestamp)() - this.sig.emp_time < this.emp_interval) && !force)
        return;
    const seq = this[FN_NEXT_SEQ]();
    const pkt = await buildLoginPacket.call(this, "wtlogin.exchange_emp", await this.tokenLogin(), 2, seq);
    try {
        let payload = await this[FN_SEND](pkt, 5, seq);
...
async function buildLoginPacket(cmd, body, type = 2, seq = this[FN_NEXT_SEQ]()) {
    var _f;
...
async function buildUniPacket(cmd, body, type = 1, seq = this[FN_NEXT_SEQ]()) {
    seq = seq || this[FN_NEXT_SEQ]();
    this.emit("internal.verbose", `send:${cmd} seq:${seq}`, VerboseLevel.Debug);
    let sec_info = constants_1.BUF0;
    if (this.signCmd.includes(cmd)) {
    ...
    switch (type) {
        case 1:
            encrypted = tea.encrypt(sso, this.sig.d2key);
            break;
        case 2:
            encrypted = tea.encrypt(sso, constants_1.BUF16);
            break;
        default:
            encrypted = sso;
    }
...
function decodeT119(t119, key = null) {
    ...
    this.sig.t10c = t[0x10c] || this.sig.t10c;
    this.sig.tgtgt = this.sig.t10c || (0, constants_1.md5)(this.sig.d2key) || constants_1.BUF0;
    ...
    this.sig.t550 = t[0x550] || this.sig.t550 || constants_1.BUF0;
    this.sig.emp_time = (0, constants_1.timestamp)();
    this.uid = "";
    if (this.sig.t543?.length) {
        try {
            let decoded = pb.decode(this.sig.t543);
            this.uid = decoded[9][11][1].toString() || "";
        }
        catch { }
    }
    if (!this.uid && this.sig.t550?.length) {
        try {
            let decoded = pb.decode(this.sig.t550);
    ...
    const token = new writer_1.default()
        ...
        .writeTlv(t119)
        .writeTlv(t119key)
        .writeTlv(this.sig.t10c || constants_1.BUF0)
