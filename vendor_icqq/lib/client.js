...
class Client extends core_1.BaseClient {
    ...
    async login(uin, password) {
        ...
        if (await this.switchQQVer()) {
            this.logger.mark(`[${uin}]获取到签名Api协议版本：${this.statistics.ver}`);
        }
        const apk_info = `${this.apk.display}_${this.apk.version}`;
        this.logger.mark(`[${uin}]使用协议：${apk_info}`);
        ...
        try {
            ...
            if (this.password_md5 && uin) {
                ...
                if (this.apk.device_type === -1) {
                    return this.logger.error("当前协议版本不支持扫码登入，请配置密码重新登入");
                }
                return this.sig.qrsig.length ? this.qrcodeLogin() : this.fetchQrcode();
            }
        }
    ...
    /** 获取用户资料卡信息 */
    async getProfile(uin) {
        return internal_1.getUserProfile.call(this, uin);
    }
    /** 设置头像 */
     ...
     */
    async getGroupShareJson(group_id) {
        const body = core_1.pb.encode({
            "1": 1,
            "2": group_id,
            "5": 2
        });
        const payload = await this.sendUni('GroupSvc.JoinGroupLink', body);
     ...
     * @param forum_id {string} 帖子id
     * @returns {string} 帖子分享url
     */
    async getForumUrl(guild_id, channel_id, forum_id) {
        const payload = await this.sendOidbSvcTrpcTcp('OidbSvcTrpcTcp.0x10c3_1', core_1.pb.encode({
            1: BigInt(guild_id),
            2: Number(channel_id),
            3: {
                1: 2,
                2: {
                    1: forum_id,
                    2: (0, common_1.timestamp)(),
                    3: BigInt(this.tiny_id)
                }
            },
            4: 1,
            9: { 1: 0 }
