...
class User extends internal_1.Contactable {
    ...
    static as(uin) {
        return new User(this, Number(uin), "");
    }
    constructor(c, uin, _uid) {
        super(c);
        this.uin = uin;
        this._uid = _uid;
        (0, common_1.lock)(this, "uin");
        (0, common_1.lock)(this, "_uid");
    }
    /** 返回作为好友的实例 */
    ...
    async getAddFriendSetting() {
        const FS = core_1.jce.encodeStruct([this.c.uin, this.uin, 3004, 0, null, 1]);
    ...
    async _getLastSeq() {
        const payload = await this.c.sendUni("trpc.msg.msg_svc.MsgService.SsoGetPeerSeq", core_1.pb.encode({
            1: this.uid
        }));
        const proto = core_1.pb.decode(payload);
        if (proto[1] > 0)
            return Number((0, common_1.timestamp)());
    ...
    async getChatHistory(time = 0, cnt = 20) {
        ...
        const body = {
            1: this.uin,
            2: Number(time > 0 ? time : (0, common_1.timestamp)()),
            3: 0,
            4: cnt > 20 ? 20 : cnt,
            5: null
        };
        if (this.c.apk.nt && this.uid?.length) {
            body[1] = this.uid;
            body[2] = Number(time > 0 ? time : await this._getLastSeq() + time);
            body[4] = cnt;
            body[5] = 2;
            const payload = await this.c.sendUni("trpc.msg.register_proxy.RegisterProxy.SsoGetRoamMsg", core_1.pb.encode(body));
            ...
            for (const proto of obj[7]) {
                try {
                    messages.push(new message_1.PrivateMessage(proto, this.c.uin, true));
                }
                catch { }
            }
            return messages;
        }
        const payload = await this.c.sendUni("MessageSvc.PbGetOneDayRoamMsg", core_1.pb.encode(body));
        const obj = core_1.pb.decode(payload);
        if (obj[1] > 0 || !obj[6])
            return messages;
    ...
    _getRouting(file = false) {
        ...
        return file ? { 15: { 1: this.uin, 2: 4 } } : { 1: { 1: this.uin } };
    }
    /**
     * 发送一条消息
     * @param content 消息内容
    ...
    async _sendMsg(proto3, brief, file = false) {
        ...
        if (rsp[1] !== 0 || rsp[14] === 0) {
            ...
            (0, errors_1.drop)(rsp[1] || errors_1.ErrorCode.RiskMessageError, rsp[2] || "私聊消息发送失败，可能被风控");
        }
        this.c.logger.info(`succeed to send: [Private(${this.uin})] ` + brief);
        this.c.stat.sent_msg_cnt++;
        const time = rsp[3];
        const message_id = (0, message_1.genDmMessageId)(this.uin, seq, rand, time, 1);
    ...
    async _sendMsgByLongMsg(converter) {
        if (!this.c.config.resend || converter.is_longMsg)
            (0, errors_1.drop)(errors_1.ErrorCode.RiskMessageError, "私聊消息发送失败，可能被风控");
...
class Friend extends User {
    ...
    get uid() {
        return this._uid || this.info?.uid || "";
    }
    /** 昵称 */
    get nickname() {
    ...
    constructor(c, uin, _info) {
        super(c, uin, _info?.uid || "");
        this._info = _info;
        (0, common_1.hide)(this, "_info");
    }
    /** 设置备注 */
    async setRemark(remark) {
        const req = core_1.jce.encodeStruct([this.uin, String(remark || "")]);
    ...
    async delete(block = true) {
        const DF = core_1.jce.encodeStruct([this.c.uin, this.uin, 2, block ? 1 : 0]);
    ...
    async searchSameGroup() {
        ...
        let res = core_1.pb.decode(payload);
        //console.log();
