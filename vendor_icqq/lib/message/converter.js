...
class Converter {
    constructor(content, ext) {
        ...
        for (let elem of _content) {
            if (!(typeof (elem) === "string" || elements_1.ChainElemTypes.includes(elem.type)) && Reflect.has(this, elem.type)) {
                _content = [elem];
                break;
            }
        }
    ...
    face(elem) {
        let { id, big } = elem;
        let faceObj = face_1.facemap[id] || Object.entries(face_1.facemap).map(([id, obj]) => {
            return { id, ...obj };
        }).find((v) => v.text.includes(id)) || { id, text: "/" + id };
        let { text, stickerId, stickerType } = faceObj;
            ...
            53: {
                ...
                2: {
                    ...
                    2: config ? {
                        1: config.unknown || 1,
                        2: config.time || 0,
                        3: config.token ? Buffer.from(config.token, "hex") : common_1.BUF0
                    } : null
                },
                3: 1
            }
