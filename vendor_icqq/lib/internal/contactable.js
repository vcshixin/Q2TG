...
class Contactable {
    ...
    async uploadImages(_imgs) {
        ...
        for (let img of _imgs) {
            ...
            else if (img instanceof message_1.Image) {
                if (img.deleteTmpFile)
                    img?.deleteTmpFile();
            }
        }
        ...
        for (let i = 0; i < imgs.length; i++) {
            ...
            const img = imgs[i];
            if (img.nt && img instanceof message_1.Image && img.nt_fileid) {
                try {
                    img.setUrl(await this.getNTPicURLbyFileid(img.nt_fileid));
                }
                catch { }
            }
            tasks.push(img.task);
    ...
    async shareUrl(content, config) {
        const body = (0, share_1.buildShare)((this.gid || this.uin), this.dm ? 0 : 1, content, config);
        await this.c.sendOidb("OidbSvc.0xb77_9", core_1.pb.encode(body));
    }
    /** 发送音乐分享 */
    async shareMusic(platform, id) {
        const body = await (0, message_1.buildMusic)((this.gid || this.uin), this.dm ? 0 : 1, platform, id);
        await this.c.sendOidb("OidbSvc.0xb77_9", core_1.pb.encode(body));
    }
    /** 发消息预处理 */
    async _preprocess(content, source) {
        try {
            ...
            const task = content
                .filter(e => !forwardNode.includes(e))
    ...
    async uploadVideo(elem) {
        ...
        return {
            type: "video",
            file: "protobuf://" + Buffer.from(buf).toString("base64"),
        };
    }
    ...
    async uploadPtt(elem, transcoding = true, brief = "") {
        ...
        return {
            type: "record",
            file: "protobuf://" + Buffer.from(core_1.pb.encode(b)).toString("base64"),
        };
    }
    async _newUploadMultiMsg(compressed) {
        ...
        if (!rsp?.[3])
            (0, errors_1.drop)(rsp?.[1], rsp?.[2]?.toString() ||
    ...
    async uploadLongMsg(content, source, isNT = this.c.apk.nt) {
        const converter = content instanceof message_1.Converter ? content : await this._preprocess(content, source);
        const seq = (0, crypto_1.randomBytes)(2).readInt16BE();
        const rand = (0, crypto_1.randomBytes)(4).readInt32BE();
        const nickname = String(this.c.nickname || this.c.uin);
        const nodes = [];
        if (isNT) {
            nodes.push({
                1: {
        ...
        const compressed = await (0, common_1.gzip)(core_1.pb.encode(isNT ? { 2: { 1: "MultiMsg", 2: { 1: nodes } } } : { 1: nodes }));
        let resid;
        try {
            resid = isNT
    ...
    async makeForwardMsg(msglist) {
        ...
        for (const fake of msglist) {
            ...
            if (fake.message.length === 1 &&
                typeof fake.message[0] !== "string" &&
                ["xml", "json"].includes(fake.message[0].type)) {
            ...
            if (!nickname && fake instanceof message_1.PrivateMessage)
                nickname =
                    this.c.fl.get(fake.user_id)?.nickname ||
                        this.c.sl.get(fake.user_id)?.nickname ||
                        nickname;
            if (cnt < 4) {
                preview.push({
                    text: `${nickname}: ${maker.brief.slice(0, 50)}`,
                });
                cnt++;
            }
            if (isNT) {
    ...
    async getForwardMsg(resid, fileName = "MultiMsg") {
        ...
        for (let proto of a) {
            try {
                ret.push(new message_1.ForwardMessage(proto, isNT));
            }
            catch { }
        }
        return ret;
    ...
    async getVideoUrl(elem, md5 = "") {
        ...
        else if (typeof md5 === 'string' && md5.substring(0, 7) === 'ntvideo') {
            return this.getNTVideoUrl({
                type: 'video',
                file: '',
                fid: fid,
                md5: md5.substring(8)
    ...
    async getNTVideoUrl(elem) {
        if (typeof elem.md5 === 'string' && elem.md5.substring(0, 7) === 'ntvideo') {
            elem.md5 = elem.md5.substring(8);
        }
        let info = {
            1: {
                1: elem.size,
        ...
        if (file.startsWith("protobuf://")) {
            ...
            info = core_1.pb.decode(buf)[2][1][1][1];
        }
        const body = core_1.pb.encode({
            1: {
                1: {
        ...
        try {
            const rsp = await this.c.sendOidbSvcTrpcTcp(this.dm ? "OidbSvcTrpcTcp.0x11e9_200" : "OidbSvcTrpcTcp.0x11ea_200", body);
            const rkey = rsp[3][1];
            if (!rkey)
    ...
    async getPttUrl(elem) {
        return elem.nt ? this.getNTPttUrl(elem) : elem.url;
    }
    async getNTPttUrl(elem) {
        ...
        if (file.startsWith("protobuf://")) {
            const buf = Buffer.from(file.replace("protobuf://", ""), "base64");
            info = core_1.pb.decode(buf)[2][1][1];
        }
        const body = core_1.pb.encode({
            1: {
                1: {
        ...
        try {
            const rsp = await this.c.sendOidbSvcTrpcTcp(this.dm ? "OidbSvcTrpcTcp.0x126d_200" : "OidbSvcTrpcTcp.0x126e_200", body);
            const rkey = rsp[3][1];
            if (!rkey)
                throw new Error("rkey不存在");
            return `https://${rsp[3][3][1]}${rsp[3][3][2]}${rkey}`;
    ...
    async getNTPicRkey() {
        ...
        const ret = {
            offNTPicRkey: "&rkey=CAQSKDOc_jvbthUjAatuFPQIo-x9wwcDhDGd8SOEu5FyJWNxNMabJTTRpO8",
            groupNTPicRkey: "&rkey=CAQSKDOc_jvbthUjZrMxJG2jNZ-rIWue47Q3PGKb_GWljzyvOUZOv0-EVao",
        };
        try {
            const rsp = await this.c.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x9067_202", body);
            const rkeys = rsp[4][1];
            if (!rkeys)
                throw new Error("rkey不存在");
            const rkeyType = {
            ...
            for (const v of rkeys) {
                ret[rkeyType[v[5]]] = v[1].toString();
            }
        }
        catch (e) {
            this.c.logger.warn("获取QQNT Rkey失败", e.message);
        }
        return ret;
    }
    /**
    ...
    async getNTPicURLbyFileid(fileid) {
        const appidInFileId = core_1.pb.decode(Buffer.from(fileid, "base64"))[4];
     ...
     * @param {string} fileid
     */
    async getNTPicInfobyFileid(fileid) {
        const proto = core_1.pb.decode(Buffer.from(fileid, "base64"));
        const info = {
        ...
        (0, child_process_1.exec)(`${ffmpeg} -y -i "${file}" -f s16le -ar 24000 -ac 1 -fs 31457280 "${tmpfile}"`, async (error, stdout, stderr) => {
            try {
                ...
                try {
                    ...
                    reject(new core_1.ApiRejection(errors_1.ErrorCode.FFmpegPttTransError, "音频转码到silk失败"));
                }
            }
            catch {
